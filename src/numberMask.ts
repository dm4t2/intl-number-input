...
export class DefaultNumberMask extends AbstractNumberMask implements NumberMask {
  conformToMask(str: string, previousConformedValue = ''): string | { fractionDigits: string; numberValue: number } {
    ...
    const checkIncompleteValue = (str: string) => {
      if (str === '' && negative && previousConformedValue !== this.numberFormat.negativePrefix) {
        ...
        if (this.numberFormat.isFractionIncomplete(str)) {
          return str
        } else if (str.startsWith(this.numberFormat.decimalSymbol as string)) {
    ...
    const [integer, ...fraction] = value.split(this.numberFormat.decimalSymbol as string)
    ...
    const invalidNegativeValue =
      integerDigits === '' && negative && (previousConformedValue === str.slice(0, -1) || previousConformedValue !== this.numberFormat.negativePrefix)
  ...
  conformToMask(str: string, previousConformedValue = ''): string | { fractionDigits: string; numberValue: number } {
    if (
      ...
      (this.numberFormat.parse(previousConformedValue) === 0 &&
        this.numberFormat.stripPrefixOrSuffix(previousConformedValue).slice(0, -1) === this.numberFormat.stripPrefixOrSuffix(str))
    ) {
      return ''
    }
    const negative = this.numberFormat.isNegative(str)
